...
-- 3.13 38. Display the name of the employee whose salary is the lowest
...
-- 3.14 39. Display the names of all employees except the lowest paid
select first_name, salary
from employees
where salary <> (select min(salary) from employees);

-- 4.1 40. Write a query to display the last name, department number, department name for all employees

select last_name, department_id, department_name
from employees join departments using (department_id);

-- 4.2 41. Create a unique list of all jobs that are in department 40. Include the location of the department in the output

select distinct(job_id), department_id, location_id
from departments join locations using (location_id) join jobs 
where department_id= 40;

-- 4.3 42. Write a query to display the employee last name,department name,location id and city of all employees who earn commission.

select last_name, department_name, location_id, city, commission_pct
from employees join departments using (department_id) join locations using (location_id)
where commission_pct is not null;

-- 4.4 43. Display the employee last name and department name of all employees who have an 'a' in their last name

select last_name, department_name
from employees join departments using (department_id)
where last_name like "%a%";

-- 4.5 44. Write a query to display the last name,job,department number and department name for all employees who work in india.
...
-- 4.6 45. Display the employee last name and employee number along with their manager's last name and manager number

select e.employee_id, e.last_name, m.employee_id, m.last_name "manager last name"
from employees as e join employees as m on e.manager_id=m.employee_id;

-- 4.7 46. Display the employee last name and employee number along with their manager's last name and manager number
...
from employees as e left join employees as m on e.manager_id=m.employee_id;
 ...
 -- 4.8 47. Create a query that displays employees last name,department number,and all the employees who work in the same 
 -- department as a given employee "hall".
 ...
 -- 4.9 48. Create a query that displays the name,job,department name,salary,grade for all employees.
--  Derive grade based on salary(>=50000=A, >=30000=B,<30000=C)
  ...
  -- 4.10 49. Display the names and hire date for all employees who were hired before their
...
-- name, emp_hire_date,manager name,man_hire_date
 ...
 -- 5.1 50. Write a query to display employee numbers and employee name (first name, last name)
...
-- 5.2 51. Fetch address details of employees belonging to the state CA. If address is null, provide default value N/A.
...
-- 5.3 52. Write a query that displays all the products along with the Sales OrderID even if an order
-- has never been placed for that product.


-- 5.4 53. Find the subcategories that have at least two different prices less than $15.

 -- 5.5 54. A. Write a query to display employees and their manager details. Fetch employee id,
...
-- 6.1 60. Write a query to display the last name and hire date of any employee in the same department as SALES.

select last_name, hire_date, department_name
from employees join departments using (department_id)
where department_name like "SALES";

-- 6.2  61. Create a query to display the employee numbers and last names of all employees who
...
-- 6.3 62. Write a query that displays the employee numbers and last names of all employees who
...
-- 6.4 63. Display the last name, department number, and job ID of all employees whose department location is america
...
-- 6.5 64. Display the last name and salary of every employee who reports to FILLMORE.

select m.last_name as "manager name",  e.salary, e.last_name as "employee name"
from employees as e join employees as m on e.manager_id=m.employee_id
where m.last_name like "King"

-- 6.6 65. Display the department number, last name, and job ID for every employee in the OPERATIONS department.

select department_id, last_name, job_id, department_name
from employees join departments using (department_id)
where department_name like "Operation%";

-- 6.7 66. Modify the above query to display the employee numbers, last names, and salaries of all
...
-- 6.8 67. Display the names of all employees whose job title is the same as anyone in the sales dept.

select concat( first_name," ",last_name) "Name", job_title
from employees join jobs using (job_id)
where job_title like "%sales%";

-- 6.9 68. Write a compound query to produce a list of employees showing raise percentages,
...
-- 6.10 69. Write a query to display the top three earners in the EMPLOYEES table. Display their last names and salaries
...
-- 6.11 70. Display the names of all employees with their salary and commission earned. Employees
...
" commission"
from employees;
-- 6.12 71. Display the Managers (name) with top three salaries along with their salaries and department information
...
-- 7.1 72. 1) Find the date difference between the hire date and resignation_date for all the
...
-- 7.2 73. Format the hire date as mm/dd/yyyy(09/22/2003) and resignation_date as mon dd,
